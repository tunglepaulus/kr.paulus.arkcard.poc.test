...
/**
 * PUT /api/accounts/update-current-company-name
 * Updates the company name of the user's current company.
  ...
  try {
    ...
    if (currentCompany) {
      ...
      const { error: updateError } = await supabase
        .from('user_company')
        .update({ company_name: trimmedCompanyName })
        .eq('id', currentCompany.id);

      if (updateError) {
        console.error('Error updating company name:', updateError);
        return NextResponse.json(
          { success: false, message: updateError.message || 'Failed to update company name' },
          { status: 500 }
        );
      }
    } else {
      ...
      if (insertError) {
        ...
        return NextResponse.json(
          { success: false, message: insertError.message || 'Failed to update company name' },
          { status: 500 }
        );
      }
    }
    ...
    console.error('Error in PUT /api/accounts/update-current-company-name:', error);
    return NextResponse.json(
